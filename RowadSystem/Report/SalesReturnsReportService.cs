...
public class SalesReturnsReportService : ISalesReturnsReportService
{
    private readonly ApplicationDbContext _context;

    public SalesReturnsReportService(ApplicationDbContext context)
    {
        _context = context;
    }

    public byte[] GenerateSalesReturnsReportPdf()
    {
        var salesReturnData = _context.SalesReturnInvoices
            .Include(si => si.Customer)
            .Include(sr => sr.salesReturnInvoiceDetails)
            .ThenInclude(srd => srd.Product) // Include Product details
            .ToList();
                    ...
                    .Column(column =>
                    {
                        column.Item()
                            ...
                            .Table(table =>
                            {
                                ...
                                foreach (var returnItem in salesReturnData)
                                {
                                    foreach (var detail in returnItem.salesReturnInvoiceDetails) // Loop over details
                                    {
                                        table.Cell().Element(CellStyle).Text(returnItem.Id.ToString());
                                        table.Cell().Element(CellStyle).Text(returnItem.Customer?.Name ?? "N/A");
                                        table.Cell().Element(CellStyle).Text(detail.Product?.Name??"No Name");
                                        table.Cell().Element(CellStyle).Text(detail.Quantity.ToString() ?? "0").AlignCenter();
    ...
    public byte[] GenerateSalesReturnsReportExcel()
    {
        var salesReturnData = _context.SalesReturnInvoices
            .Include(si => si.Customer)
            .Include(sr => sr.salesReturnInvoiceDetails)
        ...
        {
            var worksheet = workbook.AddWorksheet("Sales Returns Report");

            // Merge cells for header
            worksheet.Range("A1:E1").Merge();
            worksheet.Cell(1, 1).Value = "Rowad System - Sales Returns Report"; // Report header
            ...
            // Header for sales return details
            ...
            foreach (var returnItem in salesReturnData)
            {
                foreach (var detail in returnItem.salesReturnInvoiceDetails)
                {
                    // Fill in the return details
                    worksheet.Cell(row, 1).Value = returnItem.Id.ToString();
                    worksheet.Cell(row, 2).Value = returnItem.Customer?.Name ?? "N/A"; // Use "N/A" if null
                    worksheet.Cell(row, 3).Value = detail.Product?.Name ?? "No Name"; // Use default if null
                    worksheet.Cell(row, 4).Value = detail.Quantity.ToString() ?? "0"; // Default to 0 if null
